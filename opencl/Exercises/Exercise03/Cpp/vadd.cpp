...
int main(void)
{
    ...
    for(int x = 10; x < 1e8; x *= 10) {
    std::vector<float> h_a(x);                // a vector 
    std::vector<float> h_b(x);                // b vector 	
    std::vector<float> h_c(x);    // c = a + b, from compute device
    ...
    try 
    {
        ...
        d_c  = cl::Buffer(context, CL_MEM_WRITE_ONLY, sizeof(float) * x);
